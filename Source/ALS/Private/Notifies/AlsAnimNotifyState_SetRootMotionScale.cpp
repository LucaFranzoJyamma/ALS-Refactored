                                                       ...
                                                       const FAnimNotifyEventReference& EventReference)
{
	...
	if (IsValid(Character) && Character->GetLocalRole() >= ROLE_AutonomousProxy)
	{
		...
		{
			UE_LOG(LogAls, Warning, TEXT("%hs: The current translation scale does not match the translation scale"
				       " from the animation notify! Probably something changed it before the animation notify ended."), __FUNCTION__);
		}
	}
}
